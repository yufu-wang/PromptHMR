...
class Bedlam1PrhmrDataset(ImgfeatMotionDatasetBase):
    ...
    def __init__(
        ...
        self.smplx = make_smplx('smplx-bedlam', gender='neutral').to('cuda')
        super().__init__()

    def _load_dataset(self):
        Log.info(f"[BEDLAM-1] Loading from {self.root}")
        tic = time()
            ...
            for imf in pbar:
                ...
                for k, v in n_motion_dict.items():
                    if isinstance(v, np.ndarray):
                        n_motion_dict[k] = torch.from_numpy(v)
        ...
        Log.info(f"[BEDLAM-1] Motion files loaded. Elapsed: {time() - tic:.2f}s")

    def _get_idx2meta(self):
        # sum_frame = sum([e-s for s, e in self.mid_to_valid_range.values()])
        self.idx2meta = list(self.motion_files.keys())
        Log.info(f"[BEDLAM-1] {len(self.idx2meta)} sequences. ")
    ...
    def _process_data(self, data, idx):
        ...
        return_data = {
            "meta": {"data_name": "bedlam1", "idx": idx},
            "length": length,
