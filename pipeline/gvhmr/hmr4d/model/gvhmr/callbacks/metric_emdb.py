...
class MetricMocap(pl.Callback):
    ...
    def on_predict_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=0):
        ...
        if False:  # Render incam
            ...
            subject = video_path.split('/')[-1].replace('.mp4', '').split('_')[0]
            seqname = '_'.join(video_path.split('/')[-1].replace('.mp4', '').split('_')[1:])
            
            images_path = sorted(glob.glob(f'/home/muhammed/datasets/EMDB/{subject}/{seqname}/images/*.jpg'))
            resize_factor = 1.0
            images = read_images_np(images_path, scale=resize_factor, verbose=True)  # (F, H, W, 3), uint8, numpy
            height_img, width_img = images[0].shape[:2]
            assert width == width_img and height == height_img
