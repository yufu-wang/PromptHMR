...
class TwoWayTransformer(nn.Module):
    def __init__(
        ...
        if attention_block == 'twoway':
            attention_block = TwoWayAttentionBlock
        elif attention_block == 'cross-person' or 'cross-person' in attention_block:
            attention_block = CrossPersonAttentionBlock
...
class TwoWayAttentionBlock(nn.Module):
    ...
    def forward(
        self, queries: Tensor, keys: Tensor, query_pe: Tensor, key_pe: Tensor, **kwargs,
    ) -> Tuple[Tensor, Tensor]:
...
class CrossPersonAttentionBlock(nn.Module):
    def __init__(
        ...
        self.cross_person_attn = Attention(embedding_dim, num_heads)
        self.norm2 = nn.LayerNorm(embedding_dim)
    ...
    def forward(
        self, queries: Tensor, keys: Tensor, query_pe: Tensor, key_pe: Tensor, crossperson: bool,
    ) -> Tuple[Tensor, Tensor]:
        # Token-Prompt Attention
        if self.skip_first_layer_pe:
            queries = self.self_attn(q=queries, k=queries, v=queries)
        else:
            q = queries + query_pe
        ...
        # BUDDI style cross-person attention
        if crossperson == True:
            q = queries + query_pe
            q = rearrange(q[:,:2], 'b n d -> n b d')
            q_m = rearrange(self.pe(q), 'n b d -> 1 (b n) d')
            queries_m = rearrange(queries[:,:2], 'b n d -> 1 (b n) d')

            attn_out = self.cross_person_attn(q=q_m, k=q_m, v=queries_m)
            attn_out = rearrange(attn_out, '1 (b n) d -> b n d', n=2)
            queries[:,:2] = queries[:,:2] + attn_out
            queries = self.norm2(queries)
...
class PositionalEncoding(nn.Module):
    ...
    """
    def __init__(self, dim, max_positions=100):
